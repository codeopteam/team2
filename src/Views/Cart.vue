<template>
  ...
  <div class="flex flex-col items-center justify-center gap-7">
    ...
    <div v-if="!showModal"
      ...
      <!-- Lista de Tickets -->
      <ul v-for="(item, index) in cartStore.items" :key="index"
        ...
        <li
          ...
          <div class="ml-auto pr-8">
            <div class="flex items-center">
              <button
                class="w-10 h-14 border rounded-full text-2xl font-montserrat flex justify-center items-center leading-none border-none"
                @click="cartStore.deleteItem(index)">-
              </button>
              <span class="text-xl font-openSanz">{{ item.quantity }}</span>
              <button
                class="w-8 h-14 border rounded-full text-2xl font-montserrat flex justify-center items-center leading-none border-none"
    ...
    <div v-if="showModal"
      ...
      <div
        class="w-full max-w-xs md:w-4/6 h-24 bg-white mt-4 shadow-lg rounded-md border-t-4 border-t-yellowApp border">
...
export default {
  ...
  computed: {
    ...
    isAuthenticated() {
      const auth = getAuth();
      return !!auth.currentUser; // Retorna true si el usuario está autenticado
    },
  },
  methods: {
    async saveCartData() {
    ...
    async deleteItem(index) {
      ...
      await this.saveCartData(); // Guardar cambios después de eliminar
    },
    async addItem(item) {
